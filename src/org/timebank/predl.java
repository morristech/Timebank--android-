...
public class predl extends Activity {
...
public void onCreate(Bundle savedInstanceState) {
		...
		while (true) {
			...
			ArrayAdapter<SpinnerObject> adapter = new ArrayAdapter<SpinnerObject>(predl.this, android.R.layout.simple_spinner_dropdown_item, list);
...
mIntent1.setClass(predl.this, two.class);
mIntent1.putExtra(two.EXT_COLS, numb);
startActivity(mIntent1);
finish ();
}

private class MyThread20 extends Thread {
	@Override
	public void run() {
		try {
...
public void loadd (final int j, int g)  {
	...
	if (rez.charAt(0)=='>') {
		...
		rez1=rez;
		rez=rez.substring(rez.indexOf("<")+1, rez.length());
		Toast toast = Toast.makeText(predl.this, "Обновлено успешно", Toast.LENGTH_SHORT); 
		toast.show();}
	tableLayout=(TableLayout)findViewById(R.id.maintable);
	boolean prov = false; int st=0; String strok = ""; int prom=0; final String i1="";
	...
	for (int i = 0; (i<(rez.length())); i++)
	{
		...
		if (rez.charAt(i)=='>') {prov=true; st=i;
			...
			if (rez.charAt(i)=='<') {
                ...
                public void onClick(View v) {
                	rez=null;
                	dialog = ProgressDialog.show (predl.this, "Загрузка", "Получение данных...");
                	site = "http://timebankspb.ru/component/";
                	ter.clear();
                	ter.add(new BasicNameValuePair("view", "mobile"));
                    ter.add(new BasicNameValuePair("layout", "mobile"));
                    ter.add(new BasicNameValuePair("no_html", "1"));
                    ter.add(new BasicNameValuePair("idx", "6"));
                    ter.add(new BasicNameValuePair("no", Integer.toString(v.getId())));
                	new MyThread20().start();
                	while (rez==null) {}
                	if (rez.charAt(0)=='>') {
                	Intent mIntent = new Intent();
                    mIntent.setClass(predl.this, obj1.class);
                	...
                	finish ();} else {Toast toast = Toast.makeText(predl.this, "Проблемы с интернетом", Toast.LENGTH_SHORT); 
                	toast.show();
                	dialog.dismiss();}
                    }});
				tableRow.setOnTouchListener(new TableRow.OnTouchListener() {
		...
		Toast toast = Toast.makeText(predl.this, "Проблемы с интернетом", Toast.LENGTH_SHORT); 
		toast.show();
		rez="";
	}}
}