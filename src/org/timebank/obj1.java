...
public class obj1 extends Activity {
...
mIntent1.setClass(obj1.this, predl.class);
...
private class MyThread8 extends Thread {
	@Override
	public void run() {
		try {
...
public void loadd (int j)  {
	...
	if (rez.charAt(0)=='>') {
				...
				reson.setText(rez.substring(prom, rez.indexOf("|", prom)));
				prom=rez.indexOf("|", prom)+1;
				reson1 = (CheckBox)findViewById(R.id.checkBox1);
				if (prom!=rez.indexOf("|", prom)) {
				if (Integer.parseInt(rez.substring(prom, rez.indexOf("|", prom)))==1) {
				reson1.setChecked(true);} else {reson1.setChecked(false);}
				} else {reson1.setChecked(false);}
				prom=rez.indexOf("|", prom)+1;
				reson = (TextView)findViewById(R.id.textView7);
				reson.setText(rez.substring(prom, rez.indexOf("|", prom)));
				prom=rez.indexOf("|", prom)+1;
				...
				reson.setText(rez.substring(prom, rez.indexOf("<", prom)));
				prom=0;
				if (j==1) {Toast toast = Toast.makeText(obj1.this, "Обновлено успешно", Toast.LENGTH_SHORT); 
				toast.show();}
	} else {
		Toast toast = Toast.makeText(obj1.this, "Проблемы с интернетом", Toast.LENGTH_SHORT); 
		toast.show();
		rez="";
	}}
}